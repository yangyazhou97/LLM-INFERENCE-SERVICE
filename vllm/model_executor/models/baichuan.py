...
class BaiChuanDecoderLayer(nn.Module):
    ...
    def __init__(self, config: BaiChuanConfig, quant_config, position_embedding: str):
        super().__init__()
...
class BaiChuanModel(nn.Module):
    ...
    def __init__(self, config: BaiChuanConfig, quant_config, position_embedding: str):
        ...
        self.layers = nn.ModuleList([
            BaiChuanDecoderLayer(config, quant_config, position_embedding)
...
class BaiChuanBaseForCausalLM(nn.Module):
    ...
    def __init__(self, config, quant_config, position_embedding: str):
        super().__init__()
        self.config = config
        self.quant_config = quant_config
        self.model = BaiChuanModel(config, quant_config, position_embedding)
    ...
    def load_weights(self,
        ...
        for layer in self._column_parallel_weights:
            for suffix in weight_suffixes:
                column_parallel_weights.append(f"{layer}.{suffix}")
        row_parallel_weights: List[str] = []
        for layer in self._row_parallel_weights:
            for suffix in weight_suffixes:
        ...
        for name, loaded_weight in hf_model_weights_iterator(
            ...
            if "W_pack" in name:
                ...
                # print(f"name:{name},loaded_weight:{loaded_weight.shape},param:{param.shape}")
...
class BaichuanForCausalLM(BaiChuanBaseForCausalLM):  # baichuan 13b
    ...
    def __init__(self, config,quant_config: Optional[QuantizationConfig] = None):
        super().__init__(config, quant_config,"ALIBI")
